...
```{sql dropping_tables1, connection=mp_db_conn, echo=FALSE}
...
```{sql dropping_tables2, connection=mp_db_conn, echo=FALSE}
DROP TABLE IF EXISTS staffdose
```

```{sql dropping_tables3, connection=mp_db_conn, echo=FALSE}
...
```{r extended_function01, eval=FALSE}
mp_db_conn <- dbConnect(drv = RSQLite::SQLite(),
                        dbname = "medical_physics_db.sqlite")
