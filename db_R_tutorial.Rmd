...
You can of course automate this task to. Say you have to create a version of the sample report each year for all departments. If you already have prepared the parameterized report
...
for(year in 2021:2022) {
  rmarkdown::render(
    ...
    output_file = paste0("Sample Report for year ", year, ".html")
)
}
...
for(department in dep) {
  rmarkdown::render(
    ...
    output_file = paste0("Sample Report for year ", department, ".html")
)
}
...
```{r extendedFunction01, eval=FALSE}
...
```{r testingExtendedFunction01, eval=FALSE, echo=FALSE}
# not in tutorial-html -> Functions to help testing the extended function
mp_db_conn <- dbConnect(drv = RSQLite::SQLite(),
                        dbname = "medical_physics_db.sqlite")
