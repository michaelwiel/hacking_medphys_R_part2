...
author: |
  ...
  mchl.wieland@gmail.com, francois.gardavaud@aphp.fr
date: "`r Sys.time()`"
output: 
  html_document: 
    highlight: textmate
...
<br>
In order to make use of the report you have to execute the code of the tutorial first. This will fill the database used in the report with data, otherwise you will get empty graphs and/or error messages.

## Report Styling
As little teaser I added a logo to the report via the YAML header. There are a lot of packages that allow you to use templates for all kinds of formats. If you know your way around HMTL & CSS you can create your own templates and even write a package which adds your own template to the selection of templates when you create a new R Markdown file. The possibilities are almost endless. Here are some resources:  
...
## Graph on page 2
With the LaTeX command `\newpage`^[[R Markdown Cookbook - 4.1 Insert Page Breaks](https://bookdown.org/yihui/rmarkdown-cookbook/pagebreaks.html)] in the R Markdown file you can insert page breaks for printing the HTML-file to pdf.
